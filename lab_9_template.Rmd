...
# Observations here: Does relationship appear ~ linear? Anything else we can pick out re: trends, outliers, etc.? What is the general trend? Any outliers? Is there reason enough for us to omit it? we think it's generally linear, w/ a few outliers (from Arroyo Grande)
...
# Note: THIS SATURATED MODEL IS BAD (because PricePerSqFt is calculated from the dependent variable Price --> biased overfitting of model results)

# Note that R has selected "Foreclosure" and "Arroyo Grande" as reference levels

# Also look at the coefficients -- they don't make sense -- Bedrooms coefficient is negative (if you add a bedroom, price would decrease??) -- if lots of variables are strongly correlated (e.g., Bedrooms, Bathrooms, SqFt) (i.e., colinearity), then you get weird results --> just pick one or aggregate them into a composite variable
...
# Remember: for the CitySan Luis Obispo coefficient of 35425, then this means, all else being equal, we'd expect a house in San Luis Obispo to sell on average for $35425 more than a house in Arroyo Grande (reference level)
...
# Looks relatively homoscedastic (some outliers -- e.g., outlier #188 would be a great value house b/c it's way lower than the price prediced by the model); residuals look very normal; conceptually and mathematically, my model is making sense! 
...
# Lowest AIC = homes_lm1 --> BUT DON'T JUST PICK THE ONE WITH THE LOWEST AIC -- think about which model is better conceptually!
# So I would still pick homes_lm3
